...
case class EnsureRequirements(conf: SQLConf) extends Rule[SparkPlan] {
        ...
        case ((child, distribution), index) if childrenIndexes.contains(index) =>
          if (child.outputPartitioning.numPartitions == targetNumPartitions) {
            ...
            child match {
              ...
              case ShuffleExchangeExec(_, c, isRepartition) =>
                ShuffleExchangeExec(defaultPartitioning, c, isRepartition)
  ...
  def apply(plan: SparkPlan): SparkPlan = plan.transformUp {
    ...
    case operator @ ShuffleExchangeExec(upper: HashPartitioning, child, isRepartition) =>
